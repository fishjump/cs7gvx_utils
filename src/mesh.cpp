...
cs7gvx_utils::gl::mesh_t::mesh_t(const std::vector<vertex_t> &vertices,
...
void cs7gvx_utils::gl::mesh_t::init() {
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      _vertices.size() *
          sizeof(cs7gvx_utils::common::value_type_t<decltype(_vertices)>),
  ...
  glBufferData(
      ...
      _indices.size() *
          sizeof(cs7gvx_utils::common::value_type_t<decltype(_indices)>),
  ...
  boost::pfr::for_each_field(b, [&b](const auto &field, int index) {
    ...
    glVertexAttribPointer(index, sizeof(field) / sizeof(GL_FLOAT), GL_FLOAT,
                          GL_FALSE, sizeof(b),
                          (void *)cs7gvx_utils::common::offset_diff(field, b));
  });
}

void cs7gvx_utils::gl::mesh_t::draw(const shader_t &shader) const {
